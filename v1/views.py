...
def script():
    ...
    if user_id == 100:
        ...
        ).order_by(ApiResponse.created_at.desc()).all()[:69]
...
def run_csv():
    ...
    message = ("Data imported successully")
    return text(message)

@app.route("/run_csv_u", methods=["GET"] )
def run_csv_u():
    try:
        db.session.commit()
...
def umbrella_script(): 
    ...
    if user_id == 100:
        ...
        ).order_by(umbrellaResponse.created_at.desc()).all()[:69]
...
def run_umbrella_script():
    try:
        ...
        print(f'{business_id}')
        # Pass these IDs to the init_the_testing function
        result_content = init_the_testing(campaign_id,business_id,keywordname_id)
...
def staging_agency_script():
    ...
    if user_id == 100:
        ...
        ).order_by(stagingapiaResponse.created_at.desc()).all()[:69]
...
"""
    LDR TRAFFIC MUST HAVE
"""
@app.route("/LDR-traffic-must-haves")
@login_required
def LDR_traffic_must_haves():
    user_email = current_user.email
    user_id = current_user.id

    # Get the earliest timestamp based on the user ID
    earliest_timestamp_query = db.session.query(ldr_traffic_Model.created_at)
    if user_id != 100:
        earliest_timestamp_query = earliest_timestamp_query.filter(ldr_traffic_Model.user_id == user_id)
    earliest_timestamp = earliest_timestamp_query.order_by(ldr_traffic_Model.created_at.desc()).limit(1).scalar()

    # Retrieve traffic models
    ldr_traffic_Models_query = ldr_traffic_Model.query.filter(ldr_traffic_Model.created_at == earliest_timestamp)
    if user_id != 100:
        ldr_traffic_Models_query = ldr_traffic_Models_query.filter(ldr_traffic_Model.user_id == user_id)

    ldr_traffic_Models_list = ldr_traffic_Models_query.order_by(ldr_traffic_Model.created_at.desc()).all()

    return render_template("LDR_traffic.html", ldr_traffic_Model=ldr_traffic_Models_list, user_email=user_email)

@app.route("/LDR-traffic-must-haves-run-script", methods=["POST"])
@login_required
def LDR_traffic_must_haves_run_script():
    user_id = current_user.id
    ...
    ldr_traffic_Model_latest = ldr_traffic_Models_query.order_by(ldr_traffic_Model.created_at.desc()).first()
    try:
        last_campaing_type = ldr_traffic_Model_latest.campaign_type.split('-')[0].strip()
    except Exception as e:
        last_campaing_type = None
    if not last_campaing_type:
        last_campaing_type = 'Google Search No Click'
    current_index = constants.LDR.index(last_campaing_type)
    next_index = (current_index + 1) % len(constants.LDR)
    ldr_traffic_model_campaign_type = constants.LDR[next_index]
    try:

        test_cases = LDR_Traffic_TestCases(ldr_traffic_model_campaign_type)
        result_content = test_cases.full_dashboard_must_haves()
        db.session.commit()
        csv_file_path = "LDR_traffic_must_haves.csv"
        import_ldr_traffic_csv_to_db(db.session, csv_file_path, current_user.id, f"{ldr_traffic_model_campaign_type} - {result_content}")
